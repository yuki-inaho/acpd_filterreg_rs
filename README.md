...
`gpu` ブランチは、設計文書「三次元Coherent Point DriftのGPU向け再定式化」の中核である
**E-stepのガウス和作用素分解**をCUDAで実装し、実機で測定したものです。詳細は `docs/GPU.md`。

対応行列を作らず、前向き1成分と転置4成分を単一の作用素
`out(q,ch) = Σ_s exp(-|q-s|²/2σ²)·values(s,ch)` に帰着させています。デバイス側は全点対の
**厳密**評価で、CPU直接法との相対誤差は fp64 で 2.18×10⁻¹⁶ です。

| 測定（GTX 1070, CUDA 12.9） | 結果 |
|---|---|
| CPD方向のE-step（格子が使えない唯一の場所） | n=8000 で **100倍**（fp64）／182倍（fp32） |
| FilterReg方向のE-step | n≲5000でGPU有利、n≳16000は**CPUのO(N)格子が勝つ** |
| end-to-end `nonrigid` | n=6000 で 44.5秒 → **4.21秒** |
| 100〜1000点で最速の構成 | n≲200は純CPU、n≳300は**剛体CPU格子＋ACPD段だけGPU**の混成（n=1000で4.3倍） |

既定では選ばれません。デバイスが無ければ例外になり、CPUへ黙って切り替えることはありません。
構造化ガウス共分散・FFT格子・行列非形成PCG・GPU常駐化は**未実装**で、`docs/GPU.md` に一覧があります。
