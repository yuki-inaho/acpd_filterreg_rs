...
\end{align}
とします。3量が許容値未満となる反復が既定で5回続けば停止します。内部指標が許容値未満なら直ちに停止します。その他、最小6反復後に、明確な反発、8回の有意改善なし、反復上限、有効責任度の不足で停止します。
...
\item \code{initialization="cpd"}では、剛体更新後の点群に対しCPD式の全対平均を計算します。論文に合わせた既定です。
\item \code{initialization="filterreg"}では剛体段階の最終分散を継承します。今回の二段階統合に対する明示的な追加選択です。
\end{enumerate}

単独\code{analytic}モードには継承元がないため、明示分散なしで\code{filterreg}初期化を要求すると入力エラーです。非常に小さい剛体分散を継承すると対応が局所化し、初期内部指標より改善せず解析列が空で返る場合があります。一方CPD式の大きな初期分散は対応を広げ、既に良い剛体配置を変えてしまう場合があります。どちらかが常に有利とは主張しません。
...
C++接続は所有した数値領域へ入力をコピーした後にGILを解放し、出力領域はcapsuleの所有者が解放する設計です。Rust接続はNumPyの読取借用から所有行列へコピーし、その後\code{Python::detach}内で数値核を実行します。両方ともGILを解放した区間でPython配列の所有権に依存した生ポインタを使わない構成です。ただし、\textbf{接続の実行検証は未完了}であり、この設計説明を動作確認の代わりにしていません。\cite{nbdoc,pyo3doc,numpydoc}
...
解析最大反復・次数 & 55・1から10 & 三角形の段階長を使います。\\
解析許容値・\(w\) & \(10^{-7},0.1\) & 内部指標・相対変化、外れ値重みです。\\
解析分散初期化 & CPD式 & 明示指定、またはFilterReg継承も選べます。\\
解析分散下限 & \(10^{-12}\) & 正規化座標の二乗単位です。\\
SVD閾値・有効質量 & 各\(10^{-12}\) & 相対特異値閾値、責任度の行選択です。\\
...
R09・P1 & Python接続とpixiが未実行でした。 & \textbf{未達のままです。} 実行記録と阻害理由を残し、代替試験で合格扱いにしません。\\
...
Python経由の\textbf{151試験}は、C++試験用実行形式を明示的に使います。格子比較、事後、基底、当てはめ、twistの有限差分、次数低減、姿勢固定、保存復元、入力例外などを含みます。84条件はこれらの試験と重なるため、独立な成功例として足し合わせていません。

Rustには84参照条件と6つの性質検査、計90個の試験関数を収録しました。実nanobind/PyO3の境界用13条件、両実装の一致用8条件も用意しましたが、\textbf{いずれもこの環境では未実行}です。欠けた拡張を自動スキップして試験全体を成功表示しない設定にしています。

\section{合成データでの精度監査：悪化例も保持}\label{sec:accuracy}
原コードとの成分一致とは別に、二段階処理が実際に精度を改善するかを確認しました。2D/3Dそれぞれ3乱数種、点数180、3種類の計算（標準格子、観測専用Blur省略、直接法）、2種類の解析初期分散（CPD式、FilterReg継承）で、計36条件です。\code{probreg}自動切替はこの精度監査の36条件には含めず、成分試験で扱っています。

既知変形は第1座標に\(0.03y_2^2+0.06\)を加えるものです。剛体の初期分散は世界単位で0.08、解析の最大反復55、最大次数10です。全設定、点群生成、対応付きRMSと対称最近傍RMS、時間、終了理由を\path{tools/benchmark_synthetic.py}とJSONに保存します。

\begin{tabularx}{\textwidth}{@{}Yr@{}}
\toprule
剛体段階後との比較 & 条件数\\\midrule
対応付きRMSが\(10^{-8}\)を超えて改善 & 17\\
対応付きRMSが\(10^{-8}\)を超えて悪化 & 6\\
差が\(10^{-8}\)以内 & 13\\
数値例外 & 0\\
\bottomrule
\end{tabularx}

\subsection{悪化と改善の具体例}
標準格子・2D・乱数種90・CPD式初期分散では、対応付きRMSは初期\(0.10933\)、剛体後\(0.05664\)、解析後\(0.17449\)となりました。解析段階は55反復まで進み、\textbf{剛体段階より悪化}しています。都合の悪い例として削除したり、内部指標だけを示して成功と呼んだりしません。

同じ標準格子の3D・乱数種91・CPD式初期分散では、剛体後\(0.04799\)から解析後約\(9.79\times10^{-5}\)へ改善しました。34反復を試み、返却した写像列は最良状態の29反復分です。FilterReg分散を継承する条件では、最良状態が初期点のままで解析列0個となる例もありました。
...
D02 & PASS（実装） & 両言語に実格子を実装しました。数値比較の実行証拠はC++です。Rustの実行はD08の免除範囲です。\\
D03 & PASS（実装） & 逆事後、分散、twist、点対線・面を実装しました。有限差分等の実行証拠はC++です。\\
D04 & PASS（実装） & 無正則化の解析当てはめ、次数計画、逐次合成、最良状態を実装・照合しました。\\
D05 & PASS（C++経路） & 姿勢固定、写像再適用、保存復元、ヤコビアンを確認しました。\\
D06 & PASS & C++ビルド、原コード成分比較、異常系、メモリ検査を実行しました。\\
D07 & \textbf{BLOCKED} & nanobindがなく、実拡張の構成・ビルドに到達できていません。試験用接続は代替証拠にしません。\\
D08 & SOURCE\_ONLY & 独立Rust数値核とPyO3、90試験関数を収録・静的確認しました。コンパイルと実行必須のみ先行指示で免除されています。\\
D09 & \textbf{BLOCKED} & pixi本体がなく、依存解決を実行できていません。設定・タスク整合性の検査だけでは未達です。\\
D10 & PASS & 本LaTeX/PDF、参考資料、差分、検証と限界を記述しました。再コンパイル・全ページ描画検査の結果を配布の検証記録に保存しています。\\
D11 & PASS & クリーン展開からC++数値核100項目とPython経由151試験を再実行しました。最終版のコード指紋一致と配布内容も検査しています。\\
\bottomrule
\end{longtable}

\subsection{阻害の具体的な証拠}
実nanobind構成のCMakeを実行すると、Pythonは見つかるものの\code{No module named nanobind}で失敗しました。指定バージョンの取得も失敗しています。\code{pixi install -e cpp}は実行ファイルがなく開始できませんでした。RustコンパイラとCargoも存在しませんでした。

これらはソースの正しさを否定する試験結果ではありませんが、ビルド可能性を証明する結果でもありません。\path{validation/environment.json}には試みたコマンド、戻り値、実行ファイルの欠如、ログを記録しています。\code{pixi.lock}や\code{Cargo.lock}を架空に作成したり、未実行の継続的統合を合格扱いにしたりしません。

\textbf{従って、総合判定は未完了です。} Rustの実行免除を尊重してもD07とD09は残ります。是正したソースと検証資料は提供しますが、「DoDをすべて満たした完成版」とは表示しません。
...
環境取得、接続ビルド、各実装の一致試験を後で実行した場合は、今回のSOURCE\_ONLY/BLOCKED記録を消さず、新しい実行条件・ログ・指紋を伴う判定として追加することが、検証履歴を保つ条件です。
